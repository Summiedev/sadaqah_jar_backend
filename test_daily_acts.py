...
login_response = requests.post('https://sadaqah-jar-backend.onrender.com/auth/login', json=login_data, timeout=30)
...
if login_response.status_code == 200 and 'access_token' in login_json:
    ...
    daily_response = requests.get('https://sadaqah-jar-backend.onrender.com/sadaqah/daily', headers=headers, timeout=30)
    ...
    if daily_response.status_code == 200:
        if isinstance(daily_json, list):
            ...
            if len(daily_json) > 0:
                ...
                print(f'[OK] Daily acts endpoint is working!')
        else:
