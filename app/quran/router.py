...
_QURAN_CACHE_TTL = 60 * 60


def _cached_quran(key: str, producer):
    ...
    cache_json(key, serializable, ttl=_QURAN_CACHE_TTL)
    return data


@router.get("/surahs", response_model=Envelope)
def list_surahs(db: DbDep):
    return Envelope(data=_cached_quran("quran:surahs:v1", lambda: service.list_surahs(db)))
...
def get_surah_ayahs(surah_id: int, db: DbDep):
    ...
    return Envelope(
        data=_cached_quran(
            f"quran:surah:{surah_id}:ayahs:v1",
            lambda: service.get_surah_ayahs(db, surah_id),
        )
    )
...
def get_page(page_number: int, db: DbDep):
    ...
    return Envelope(
        data=_cached_quran(
            f"quran:page:{page_number}:v1", lambda: service.get_page(db, page_number)
        )
    )
...
def get_juz(juz_number: int, db: DbDep):
    ...
    return Envelope(
        data=_cached_quran(
            f"quran:juz:{juz_number}:v1", lambda: service.get_juz(db, juz_number)
        )
    )
...
def get_hizb(hizb_number: int, db: DbDep):
    ...
    return Envelope(
        data=_cached_quran(
            f"quran:hizb:{hizb_number}:v1", lambda: service.get_hizb(db, hizb_number)
        )
    )
...
def search(
    ...
    return Envelope(
        data=_cached_quran(
            f"quran:search:{normalized}:{limit}:v1",
