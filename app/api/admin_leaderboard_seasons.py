...
router = APIRouter(prefix="/admin/leaderboard-seasons", tags=["Admin Leaderboard Seasons"])
...
@router.get("/")
def list_leaderboard_seasons(db: Session = Depends(get_db), admin=Depends(require_admin)):
    rows = db.query(LeaderboardSeason).order_by(LeaderboardSeason.season_name.asc()).all()
...
def delete_leaderboard_season(season_name: str, db: Session = Depends(get_db), admin=Depends(require_admin)):
    season = get_leaderboard_season(db, season_name)
    if season is None:
