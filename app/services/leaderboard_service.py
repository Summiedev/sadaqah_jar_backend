...
def increment_global(user_id: int, stars: int):
    try:
        ...
        logger.warning(
            "Failed to increment global leaderboard for user %s: %s", user_id, exc
        )
...
def increment_ramadan(user_id: int, stars: int):
    try:
        ...
        logger.warning(
            "Failed to increment ramadan leaderboard for user %s: %s", user_id, exc
        )
...
def increment_weekly(user_id: int, stars: int):
    try:
        ...
        logger.warning(
            "Failed to increment weekly leaderboard for user %s: %s", user_id, exc
        )
...
def increment_family_leaderboard(jar_id: int, user_id: int, stars: int):
    try:
        ...
        logger.warning(
            "Failed to increment family leaderboard for jar %s user %s: %s",
            jar_id,
            user_id,
            exc,
        )
...
def get_top_family(jar_id: int, limit: int = 10):
    try:
        return redis_client.zrevrange(
            get_family_key(jar_id), 0, limit - 1, withscores=True
        )
    except redis.RedisError as exc:
...
def increment_friday(user_id: int, stars: int):
    try:
        ...
        logger.warning(
            "Failed to increment friday leaderboard for user %s: %s", user_id, exc
        )
