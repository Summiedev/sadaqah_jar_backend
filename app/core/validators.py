...
def validate_pagination(limit: int | None, offset: int | None, max_limit: int = 100) -> dict[str, int]:
    limit = min(limit or 20, max_limit)
    offset = max(offset or 0, 0)
    return {"limit": limit, "offset": offset}


def validate_date_range(start: datetime | None, end: datetime | None) -> tuple[datetime | None, datetime | None]:
