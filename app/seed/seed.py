...
class DatabaseSeeder:
    ...
    def seed_users(self, count: int = 10) -> list[User]:
        ...
        for i in range(count):
            username, email = (
                names[i] if i < len(names) else (f"user{i}", f"user{i}@test.com")
            )
    ...
    def seed_jars(self, users: list[User]) -> list[Jar]:
        ...
        for user in users:
            jar = Jar(
                ...
                else datetime.utcnow() - timedelta(days=random.randint(1, 60)),
            )
            self.db.add(jar)
            jars.append(jar)
    ...
    def seed_sadaqah_logs(
        self, users: list[User], acts: list[SadaqahAct], days_back: int = 60
    ):
    ...
    def seed_family_jars(self, users: list[User]) -> list[FamilyJar]:
        ...
        for family_jar in family_jars:
            creator = (
                self.db.query(User).filter(User.id == family_jar.created_by).first()
            )
    ...
    def seed_family_logs(
        self, family_jars: list[FamilyJar], acts: list[SadaqahAct], days_back: int = 30
    ):
        ...
        for family_jar in family_jars:
            ...
            members = (
                self.db.query(FamilyJarMember)
                .filter(FamilyJarMember.family_jar_id == family_jar.id)
                .all()
            )
